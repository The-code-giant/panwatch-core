...
- `Symbol(market: Market, code)` —— `Symbol.parse("600519")`(自动识别)/ `Symbol.parse("00700", "HK")`;`.to_tencent()` / `.to_eastmoney_secid()` / `.to_yfinance()`。`Market` = `US` / `CA`(Yahoo 形式 `SHOP.TO` / `XYZ.V`)/ `CRYPTO` / `GOLD`(`CN` / `HK` 为遗留)。`Symbol.parse("SHOP.TO")` 自动识别为 `CA`,`.to_yfinance()` 原样返回。
