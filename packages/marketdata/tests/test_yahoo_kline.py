...
def test_yahoo_kline_parses_ca(monkeypatch):
    """Canadian symbols (SHOP.TO) route to Yahoo unchanged and the vendor accepts the CA market."""
    payload = _chart_payload([_TS1], [150.0], [155.0], [149.0], [154.0], [500000])
    captured = {}
    ...
    assert "CA" in kv.YahooKlineVendor.supports_markets
    out = kv.YahooKlineVendor().fetch([Symbol.parse("SHOP.TO")], {"days": 60})
    assert captured["url"].endswith("/SHOP.TO")
    assert len(out) == 1 and out[0].close == 154.0
