...
class TestPortfolioContext(unittest.TestCase):
    ...
    def test_with_position_renders_holding_info(self):
        ...
        self.assertIn("Trading style", text)
        self.assertIn("Long-term", text)  # trading_style="long" 映射后的展示文案
...
class TestAgentCollect(unittest.IsolatedAsyncioTestCase):
    async def test_collect_from_marketdata_package(self):
        """collect() — 走 marketdata 包(quote→dict / fundamentals→list / holders→list / financial 摘要)"""
        from datetime import datetime as _dt

        from src.agents.tradingagents import agent as agent_module
        from marketdata import Bar, EventItem, Fundamentals, HolderItem, Quote
        ...
        fake_quote = Quote(symbol="AAPL", market="US", current_price=225.30, name="Apple Inc.")
        fake_bar = Bar(date="2026-05-15", open=223.0, close=225.30, high=226.4, low=222.1, volume=48_000_000.0)
        fake_fund = Fundamentals(
            symbol="AAPL", market="US", name="Apple Inc.", pe_ttm=31.5, revenue=391_035_000_000.0,
            net_profit=93_736_000_000.0, report_date="2025-06-28", currency="USD",
        )
        fake_holder = HolderItem(symbol="AAPL", kind="breakdown", holder="Insiders % held", pct_out=0.07, source="yfinance")
        fake_event = EventItem(
            source="sec_edgar", external_id="1", event_type="filing", title="Form 10-Q",
            publish_time=_dt.now(), symbols=["AAPL"], importance=1, url="",
        )

        fake_md = MagicMock()
        fake_md.quotes = MagicMock(return_value=[fake_quote])
        fake_md.klines = MagicMock(return_value=[fake_bar])
        fake_md.fundamentals = MagicMock(return_value=[fake_fund])
        fake_md.holders = MagicMock(return_value=[fake_holder])
        ...
        fake_md.fundamentals.assert_called_once_with(["AAPL"], market="US")
        fake_md.holders.assert_called_once_with(["AAPL"], market="US")
        self.assertNotIn("capital_flow", data)

        self.assertEqual(data["stock"], stock)
        self.assertIsInstance(data["quote"], dict)
        self.assertEqual(data["quote"]["current_price"], 225.30)
        self.assertEqual(data["fundamentals"], [fake_fund])
        self.assertEqual(data["holders"], [fake_holder])
        self.assertEqual(data["financial"]["periods"], ["2025-06-28"])
        self.assertEqual(data["financial"]["currency"], "USD")
        self.assertEqual(data["financial"]["indicators"]["Total Revenue"], {"2025-06-28": 391_035_000_000.0})
        self.assertEqual(data["financial"]["indicators"]["P/E (TTM)"], {"2025-06-28": 31.5})
        self.assertNotIn("Total Debt", data["financial"]["indicators"])
    ...
    async def test_collect_partial_failure_keeps_other_data(self):
        """collect() — 单个数据源抛异常只丢该项(financial 为空壳),其余数据保留"""
        from src.agents.tradingagents import agent as agent_module
        from marketdata import Quote

        agent = TradingAgentsAgent()
        stock = MagicMock()
        stock.symbol = "SHOP.TO"
        stock.name = "Shopify Inc."
        stock.market = MagicMock()
        stock.market.value = "CA"
        context = MagicMock()
        context.watchlist = [stock]

        fake_md = MagicMock()
        fake_md.quotes = MagicMock(return_value=[Quote(symbol="SHOP.TO", market="CA", current_price=150.25, name="Shopify Inc.")])
        fake_md.klines = MagicMock(return_value=[])
        fake_md.fundamentals = MagicMock(side_effect=RuntimeError("yahoo down"))
        fake_md.holders = MagicMock(return_value=[])
        fake_md.events = MagicMock(return_value=[])

        with patch.object(agent_module, "get_market_data", lambda: fake_md):
            data = await agent.collect(context)

        self.assertEqual(data["quote"]["current_price"], 150.25)
        self.assertEqual(data["fundamentals"], [])
        self.assertEqual(data["holders"], [])
        self.assertEqual(data["financial"], {"periods": [], "currency": "", "indicators": {}, "categories": {}})
