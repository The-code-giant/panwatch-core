...
def test_benchmark_result_cached(db, monkeypatch):
    ...
    monkeypatch.setattr(accounts_api, "_gather_holdings", lambda d: (list(_HOLDINGS), dict(_COVERAGE)))
    ...
    # The additive `valuation` coverage block (REVIEW-04 §A) rides along with the
    # cached result; it must be identical on the cache hit, not merely present.
    assert r1 == r2 == {"excess_return": 1.23, "valuation": _COVERAGE}
...
def test_benchmark_empty_not_cached(db, monkeypatch):
    ...
    # Coverage still rides along on the uncached "insufficient_data" branch.
    assert r1.get("valuation") == _COVERAGE
...
def test_attribution_result_cached(db, monkeypatch):
    ...
    assert r1 == r2 == {
        "items": [{"symbol": "600519", "contribution_pct": 1.0}],
        "valuation": _COVERAGE,
    }
