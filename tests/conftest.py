...
的构造就已经把 DATA_DIR / PANWATCH_ENV_FILE 读进去了；等到某个 fixture 才去 setattr
...
东西之前完成:
  - DATA_DIR 指向一个每次 pytest 会话都新建的一次性目录,而不是仓库里的 ./data —
    ...
    目录,不会碰真实安装数据。
  - PANWATCH_ENV_FILE 指向同一个一次性目录里一个必然不存在的文件名,这样
...
if "src.config" in sys.modules or "src.web.database" in sys.modules:
    raise RuntimeError("PanWatch configuration was imported before pytest isolation")
...
_PYTEST_DISPOSABLE_DIR = tempfile.mkdtemp(prefix="panwatch-pytest-")
os.environ["DATA_DIR"] = _PYTEST_DISPOSABLE_DIR
os.environ["PANWATCH_ENV_FILE"] = os.path.join(
