...
def test_build_portfolio_benchmark_with_mocked_fetch(monkeypatch):
    ...
    res = pb.build_portfolio_benchmark(
        [{"symbol": "AAPL", "market": "US", "quantity": 100, "fx": 1.0}],
        days=60,
        benchmark_code="GSPC",
        kline_fetch=fake_fetch,
    )
    assert res is not None
    assert res["benchmark_code"] == "GSPC"
    assert res["benchmark_label"] == "S&P 500"
...
def test_default_benchmark_is_sp500_and_tsx_is_available():
    """The default benchmark is the S&P 500 and the S&P/TSX Composite is selectable; labels are English."""
    assert pb.DEFAULT_BENCHMARK == "GSPC"
    assert pb.benchmark_label("GSPC") == "S&P 500"
    assert pb.benchmark_label("^GSPC") == "S&P 500"
    assert pb.benchmark_label("GSPTSE") == "S&P/TSX Composite"
    assert pb.BENCHMARKS["GSPTSE"][2] == "CA"


def test_fetch_benchmark_series_uses_index_klines(monkeypatch):
    """The benchmark series is fetched through kline_collector.get_index_klines (Yahoo-first US/CA index path)."""
    from src.collectors import kline_collector

    captured = {}

    def _fake(code, market, days=120):
        captured["code"], captured["market"], captured["days"] = code, market, days
        return _bars([("2026-01-02", 100.0), ("2026-01-03", 101.0)])

    monkeypatch.setattr(kline_collector, "get_index_klines", _fake)
    dates, closes = pb._fetch_benchmark_series("GSPTSE", 60)
    assert captured["code"] == "GSPTSE" and captured["market"].value == "CA"
    assert dates == ["2026-01-02", "2026-01-03"] and closes == [100.0, 101.0]
