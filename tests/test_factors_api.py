...
def test_list_weights_returns_all_market_factor_pairs():
    """GET 列表返回 5 因子 × 已启用市场数(US/CA → 10)。"""
    from src.core.factor_weights import CALIBRATABLE_FACTORS, MARKETS
    ...
    try:
        ...
        assert "items" in res
        assert len(res["items"]) == len(CALIBRATABLE_FACTORS) * len(MARKETS)
    finally:
        db.close()
...
def test_update_weight_pins_and_sets_value():
    ...
    try:
        ...
        res = factors.update_weight("alpha_score", "US", payload, db=db)
...
def test_update_weight_unknown_factor_returns_400():
    ...
    try:
        ...
        with pytest.raises(HTTPException) as ei:
            factors.update_weight("bad_factor", "US", payload, db=db)
...
def test_update_weight_disabled_market_returns_400():
    """A retired market (CN) is not a valid target → HTTP 400."""
    from src.web.api import factors
    ...
    try:
        payload = factors.FactorWeightUpdate(weight=1.1)
        with pytest.raises(HTTPException) as ei:
            factors.update_weight("alpha_score", "CN", payload, db=db)
        assert ei.value.status_code == 400
    finally:
        db.close()
