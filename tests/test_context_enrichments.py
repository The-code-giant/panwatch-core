...
def test_relative_strength_computes_excess():
    ...
    assert rs["index_label"]  # non-empty label (S&P 500 etc.)
...
def test_index_symbol_map_covers_markets():
    """Every enabled equity market maps to an index code + label; retired markets fall back to the default."""
    from src.models.market import EQUITY_MARKETS

    cb = ContextBuilder()
    for mkt in EQUITY_MARKETS:
        sym, label = cb._index_for_market(MarketCode(mkt))
        assert sym
        assert label
    assert cb._index_for_market(MarketCode.US) == (".INX", "S&P 500")
    assert cb._index_for_market(MarketCode.CA) == ("GSPTSE", "S&P/TSX Composite")
    # retired CN is not mapped → default market's index
    assert cb._index_for_market(MarketCode.CN) == cb._index_for_market(MarketCode.US)
...
def test_build_symbol_contexts_injects_new_keys(monkeypatch):
    """端到端:payload 同时带 ta_verdict / relative_strength / events(原样透传,无全文注入)。"""
    ...
    assert rs["index_label"] == "S&P 500"  # retired CN rows fall back to the default (US) index
    # ① 事件原样透传(公告全文注入已随东财源一起退役)
    assert payload["events"][0]["title"] == "重大资产重组预案"
