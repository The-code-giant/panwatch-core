...
def test_keyword_news_formats():
    """Industry/theme keyword search (Google News): the reply carries the keyword and the headline."""
    from unittest.mock import patch
    from datetime import datetime
    from src.collectors.news_collector import NewsItem

    def fake(kw, market="US"):
        return [NewsItem(source="em", external_id="1", title=f"{kw} sector update", content="", publish_time=datetime(2026, 5, 30))]
    with patch("src.core.marketdata_client.md_news_by_keyword", fake):
        r = ta._serve_keyword_news("semiconductors")
    assert "semiconductors" in r
