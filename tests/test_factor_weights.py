...
def test_get_factor_weights_idempotent_no_dup_rows():
    ...
    db = _mem_db()
    try:
        get_factor_weights("US", db=db)
        get_factor_weights("US", db=db)
        n = db.query(FactorWeight).filter(FactorWeight.market == "US").count()
...
def test_get_factor_weights_reads_stored_value():
    ...
    db = _mem_db()
    try:
        db.add(FactorWeight(factor_code="alpha_score", market="US", weight=1.3))
        db.commit()
        w = get_factor_weights("US", db=db)
...
def test_get_all_factor_weights_lists_all_markets():
    ...
    try:
        ...
        assert ("alpha_score", "US") in keys
        assert ("alpha_score", "CA") in keys
        assert not any(m in ("CN", "HK") for _, m in keys)
        sample = items[0]
...
def test_set_factor_weight_manual_writes_history():
    ...
    db = _mem_db()
    try:
        res = set_factor_weight("alpha_score", "US", weight=1.3, is_pinned=True, db=db)
        assert res["weight"] == 1.3
        assert res["is_pinned"] is True
        hist = (db.query(FactorWeightHistory)
                .filter_by(factor_code="alpha_score", market="US", reason="manual").all())
        assert len(hist) == 1
...
def test_set_factor_weight_rejects_unknown_factor():
    ...
    try:
        with pytest.raises(ValueError):
            set_factor_weight("not_a_factor", "US", weight=1.1, db=db)
    finally:
        db.close()
