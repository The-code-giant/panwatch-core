...
def test_evaluate_factor_ic_filters_by_market():
    ...
    try:
        d = _old_date()
        for i in range(1, 6):  # 5 US
            _seed_pair(db, i, market="US", snapshot_date=d, alpha=float(i), ret=float(i))
        for i in range(6, 10):  # 4 CA
            _seed_pair(db, i, market="CA", snapshot_date=d, alpha=float(i), ret=float(i))
        db.commit()

        res = evaluate_factor_ic(days=90, horizon=5, min_samples=3, market="US", db=db)
...
def test_evaluate_factor_ic_excludes_unelapsed_horizon():
    ...
    try:
        for i in range(1, 5):  # 4 条足够老
            _seed_pair(db, i, market="US", snapshot_date=_old_date(), alpha=float(i), ret=float(i))
        # 1 条「今天」的泄漏样本
        _seed_pair(db, 99, market="US", snapshot_date=date.today().strftime("%Y-%m-%d"),
                   alpha=9.0, ret=9.0)
        db.commit()
...
def test_calibrate_moves_weight_from_ic_and_audits():
    ...
    try:
        ...
        calibrate_factor_weights("US", min_samples=5, db=db)

        row = db.query(FactorWeight).filter_by(factor_code="alpha_score", market="US").first()
        assert row.weight > 1.0
        assert abs(row.weight - 1.14) < 0.02

        hist = (db.query(FactorWeightHistory)
                .filter_by(factor_code="alpha_score", market="US", reason="auto").all())
        assert len(hist) == 1
...
def test_calibrate_skips_pinned():
    ...
    db = _mem_db()
    try:
        db.add(FactorWeight(factor_code="alpha_score", market="US", weight=1.0, is_pinned=True))
        db.commit()
        d = _old_date()
        for i in range(1, 7):
            _seed_pair(db, i, market="US", snapshot_date=d, alpha=float(i), ret=float(i))
