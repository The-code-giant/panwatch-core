...
def test_get_index_klines_unknown_returns_empty(monkeypatch):
    """An index with no mapping anywhere → empty list, fail-soft, no exception."""
    from src.collectors import kline_collector
    ...
    monkeypatch.setattr(kline_collector, "get_market_data", lambda: _MD())
    assert kline_collector.get_index_klines("NOPE", MarketCode.US) == []


def test_fetch_index_context_us_failsoft(monkeypatch):
    """When the index source yields nothing, _fetch_index_context returns available False without raising."""
    from src.collectors import kline_collector
    from src.core.context_builder import ContextBuilder

    monkeypatch.setattr(kline_collector, "get_index_klines", lambda *a, **k: [])
