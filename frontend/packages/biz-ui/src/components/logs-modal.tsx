...
export default function LogsModal({ open, onOpenChange }: { open: boolean, onOpenChange: (v: boolean) => void }) {
  ...
  useEffect(() => {
    if (!open) return
    loadLatest()
    // query is triggered via handleSearchInput's debounce, avoiding an immediate request on every keystroke.
  }, [open, selectedLevels, selectedLoggers, selectedFlow, domain, timeRange])

  // Auto-refresh: prefer SSE tail (server pushes increments, event id is the log id, auto-resumes after disconnect),
  // falls back to the original 3s polling if SSE is unavailable/the stream closes (polling code kept as a fallback)
  useEffect(() => {
    if (!(open && autoRefresh)) {
    ...
    const close = subscribeSSE(`/logs/stream?${params.toString()}`, {
      onEvent: (ev) => {
        if (ev.event === 'logs' && Array.isArray(ev.data?.items)) {
          ...
          setLogs(prev => {
            const seen = new Set(prev.map(x => x.id))
            // Server pushes in ascending id order, but the list shows newest first → reverse then prepend
      ...
      // Server stream closed normally on timeout / connection retries exhausted → fall back to polling
      ...
      <DialogContent className="w-[90vw] max-w-[90vw] h-[90vh] max-h-[90vh] flex flex-col overflow-hidden" onInteractOutside={(e) => e.preventDefault()}>
        ...
        <div className="card p-3 md:p-4 mb-3 space-y-3">
          <div className="relative">
            ...
            <Input value={query} onChange={e => handleSearchInput(e.target.value)} placeholder="Search log message / trace_id / logger..." className="pl-10" />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <span className="ml-auto text-[11px] text-muted-foreground font-medium">{total} entries</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-[11px] text-muted-foreground">Default flows automatically include `src.agents.base` decision logs</div>
          </div>
        ...
        <div className="flex-1 min-h-0">
          {!loadedOnce && loading ? (
            ...
            <div className="card flex flex-col items-center justify-center py-20">
              ...
              <p className="text-[15px] font-semibold text-foreground">No logs yet</p>
              <p className="text-[13px] text-muted-foreground mt-1.5">Logs will appear here automatically once the backend is running</p>
            </div>
          ) : (
              ...
              <div className="overflow-x-auto overflow-y-auto flex-1 min-h-0 relative scrollbar">
                ...
                {loading && loadedOnce && (
                  <div className="absolute top-2 right-4">
                    <span className="w-4 h-4 border-2 border-border border-t-muted-foreground rounded-full animate-spin inline-block" />
                  </div>
                )}
              </div>
