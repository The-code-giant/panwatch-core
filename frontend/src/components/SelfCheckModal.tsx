...
const STATUS_META: Record<RowStatus, {
  ...
  fail: { label: 'Down', variant: 'destructive', className: '', Icon: XCircle },
}
...
/** Groups by provider (group), preserving encounter order. */
...
export default function SelfCheckModal({ open, onClose }: SelfCheckModalProps) {
  // First render a grouped skeleton from the checklist (all "checking"), then backfill each item's status as results arrive.
  ...
  const runCheck = useCallback(async () => {
    ...
    try {
      ...
      setListError(e instanceof Error ? e.message : 'Failed to fetch self-check list')
      setRunning(false)
      return
    }
    if (runId !== runIdRef.current) return
      ...
      while (true) {
        ...
        try {
          ...
          merge(it.key, probed
            ...
            : { status: 'fail', error: 'No result returned', hint: 'Check request failed, try again later' })
        } catch (e) {
          merge(it.key, { status: 'fail', error: e instanceof Error ? e.message : 'Request failed', hint: 'Check request failed, try again later' })
        }
      }
    }
    await Promise.all(
  ...
  const heroGradient = 'bg-rail text-rail-foreground'
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        ...
        {/* Gradient hero: progress bar + total/OK/failed counts */}
        <div className={`relative overflow-hidden rounded-2xl ${heroGradient} p-4`}>
          <div className="flex items-center justify-between gap-2">
          ...
          {finished && failCount > 0 && (
            <div className="mt-3 rounded-lg bg-white/15 px-3 py-1.5 text-[11px]">
              Found {failCount} issue(s) - see the fix suggestions below.
            </div>
          )}
        </div>
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <label className="flex items-center gap-2 text-[12px] text-muted-foreground cursor-pointer select-none">
            ...
            Include real notification sends
          </label>
        ...
        {!listError && total === 0 && !running && (
          <div className="mt-4 rounded-xl border border-border/40 bg-accent/20 p-4 text-center text-[12px] text-muted-foreground">
            No data source / AI / notification configured - set those up first, then run the self-check.
          </div>
        )}

        {/* Grouped details: Data Sources / AI Models (provider → model) / Notification Channels */}
