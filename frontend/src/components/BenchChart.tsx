...
import type { BenchmarkCurvePoint } from '@tickerkeep/api'
...
// 3 internal reference lines (evenly spaced, not the top/bottom border lines)
...
function BenchChartSvg({
  ...
  const padLeft = 2
  const padRight = 40 // reserve space on the right for the % axis labels
  const padTop = 12
  const padBottom = 12
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Portfolio NAV vs. benchmark chart">
      ...
      {/* Grid lines + right-side % axis (cumulative return relative to the curve's starting point = 100) */}
      {GRID_FRACS.map((frac) => {
      ...
      {/* Benchmark: dashed line (neutral color) */}
      <polyline
      ...
      {/* Portfolio: solid line + light area fill (the protagonist) */}
      ...
      {/* End-point circles (start/end of the portfolio line) */}
...
/**
 * Portfolio NAV vs. benchmark dual-line chart, no third-party charting library dependency.
 * Portfolio (primary) is a solid line + light area fill; benchmark is a dashed neutral-colored line; 3 dashed
 * gridlines + right-side % axis; circles at both ends of the portfolio line.
 * Renders the SVG using the container's clientWidth (measured via ResizeObserver) rather than CSS scaling, so
 * axis text/line widths don't distort as the width changes.
 * Renders nothing when curve is empty or has fewer than 2 valid points (the caller is responsible for showing a
 * "computing" placeholder, etc.).
 */
