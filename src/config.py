...
class Settings(BaseSettings):
    ...
    # AI
    # No default endpoint: an unconfigured install must not send prompts
    # anywhere. Set AI_BASE_URL/AI_MODEL, or configure a provider in the UI.
    ai_base_url: str = ""
    ai_api_key: str = ""
    ai_model: str = ""
    ...
    # 默认时区（用于调度、时间展示等）。
    # 统一使用一个环境变量控制：TZ（默认 America/Vancouver）。
    # 建议使用 IANA 时区名，如 America/Vancouver, America/New_York, America/Toronto。
