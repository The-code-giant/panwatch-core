...
# Long-form market labels for the LLM context (legacy codes kept for gated old rows).
_MARKET_LABELS = {
    "US": "US stocks (NYSE/Nasdaq)",
    "CA": "Canadian stocks (TSX/TSXV)",
    "CRYPTO": "Crypto",
    "GOLD": "Gold",
    "CN": "China A-shares",
    "HK": "Hong Kong stocks",
...
def build_stock_metadata_context(
    ...
    LLMs must not guess the company from the ticker (Canadian ``.TO`` / ``.V`` symbols
    and legacy numeric codes are easy to misread), so the name is stated explicitly.
    ...
    lines.append(
        "- IMPORTANT: DO NOT guess the company from the ticker code; always use the "
        "company name above."
    )
    return "\n".join(lines)
...
def build_portfolio_context(
    ...
    Args:
        ...
        stock_market: 当前分析标的的市场代码(可选)。传入时用于把持仓匹配限定在
            同一市场,避免不同市场下同代码的持仓互相污染该标的的推荐。
    ...
    try:
        positions = portfolio.get_positions_for_stock(stock_symbol, stock_market)
    except Exception:
        positions = []
    ...
    # Read separately and coerce: a portfolio-like object that does not expose the
    # unsupported-exposure API must not blank out the real totals read above.
    try:
        unsupported_cost = float(portfolio.unsupported_cost or 0.0)
        unsupported_count = len(portfolio.unsupported_positions)
    except Exception:
        unsupported_cost = 0.0
        unsupported_count = 0
    ...
    if positions:
        ...
        style_map = {"short": "Short-term", "swing": "Swing", "long": "Long-term"}
        style_label = "/".join(style_map.get(s, s) for s in styles) if styles else "Swing"
    ...
    if total_available > 0 or total_cost > 0:
        ...
        if unsupported_cost > 0:
            lines.append(
                f"- Unanalysed exposure: {unsupported_cost:.2f} across {unsupported_count} "
                f"holding(s) in a market this deployment does not currently analyse. This "
                f"amount IS included in the totals above but is EXCLUDED from this analysis "
                f"— it is not cash and carries real market risk."
            )
