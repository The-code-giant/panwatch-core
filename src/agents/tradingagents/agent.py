...
class TradingAgentsUnsupportedMarket(RuntimeError):
    """标的市场未启用(disabled market,含 CN/HK 等已退役市场,或
    ``MarketCode.UNKNOWN`` 占位符)。REVIEW-04.md §C 要求:入口必须在触碰任何
    provider/model 之前拒绝执行,而不是先跑一轮 collect/analyze 再失败。"""


class TradingAgentsAgent(BaseAgent):
    name = "tradingagents"
    display_name = "TradingAgents Deep Analysis"
    description = "Multi-agent investment decision framework, 3-5 minutes, ~$0.05/run (deepseek-chat)"
    ...
    def __init__(
        ...
        if invalid:
            raise ValueError(
                f"Invalid analyst name(s): {invalid}; "
    ...
    async def collect(self, context: AgentContext) -> dict:
        ...
        # 5 类并发拉;单项失败只丢该项(return_exceptions),不拖垮整轮
        results = await asyncio.gather(
            asyncio.to_thread(md.quotes, [sym], market=mkt),
            asyncio.to_thread(md.klines, sym, market=mkt, days=120),
            asyncio.to_thread(md.fundamentals, [sym], market=mkt),
            asyncio.to_thread(md.holders, [sym], market=mkt),
            asyncio.to_thread(md.events, [sym], market=mkt, since_days=30),
            return_exceptions=True,
        )
        labels = ("quotes", "klines", "fundamentals", "holders", "events")
        cleaned: list[list] = []
        for label, r in zip(labels, results):
            if isinstance(r, BaseException):
                logger.warning(f"[TA] 数据收集部分失败 ({label}): {r}")
                cleaned.append([])
            else:
                cleaned.append(list(r or []))
        quotes, klines_list, fund_list, holders_list, events_list = cleaned
        quote_dict = _quote_to_row(quotes[0]) if quotes else {}

        # 财务摘要:所有市场统一走 marketdata Fundamentals(Yahoo Finance / SEC EDGAR)
        from src.agents.tradingagents.financial_data import (
            build_financial_from_fundamentals,
            empty_financial,
        )
        financial: dict = (
            build_financial_from_fundamentals(fund_list[0]) if fund_list else empty_financial()
        )
    ...
    async def analyze(self, context: AgentContext, data: dict) -> AnalysisResult:
        ...
        # 市场闸门:disabled/unknown 市场必须在任何 provider/model 调用之前拒绝执行
        # (REVIEW-04.md §C)。放在 cache 命中检查 / 预算查询 / _run_tradingagents_sync
        # 之前,确保拒绝路径完全不触发数据库/AI 客户端/collector 调用。
        if not is_enabled(stock.market):
            market_label = (
                stock.market.value if isinstance(stock.market, MarketCode) else str(stock.market)
            )
            raise TradingAgentsUnsupportedMarket(
                f"TradingAgents does not run for disabled/unknown market: {market_label}"
            )
        ...
        if budget["exceeded"]:
            if self.over_budget_action == "reject":
                raise RuntimeError(
                    f"This month's TradingAgents budget is exhausted "
                    f"(${budget['used']:.2f} / ${self.monthly_budget_usd:.2f}). "
                    f"To keep using it, raise the budget limit in Settings."
                )
        ...
        try:
            ...
            logger.warning(
                f"[TA] Execution timed out (>{self.timeout_minutes} min). "
                f"Completed stages: {partial_stages}, total cost ${partial_cost:.4f}"
            )
            partial_msg = (
                f"Analysis timed out (>{self.timeout_minutes} min). "
                f"Completed {len(partial_stages)} stage(s), total cost ${partial_cost:.4f}. "
                f"Suggestions: ① shorten debate_rounds; ② switch to a faster model (e.g. deepseek-chat); "
                f"③ increase timeout_minutes."
            )
        ...
        try:
            ...
            save_suggestion(
                ...
                agent_name=self.name,
                agent_label="TradingAgents Deep",
                signal=signal_text,
                reason=reason_text,
    ...
    def _check_availability(self) -> tuple[bool, str]:
        ...
        try:
            ...
            return False, (
                ...
                "(or install it separately with `pip install \"tradingagents @ git+https://github.com/TauricResearch/TradingAgents.git\"`). "
                "If this fails behind a corporate proxy, try `env -u HTTP_PROXY -u HTTPS_PROXY pip install -r requirements.txt`. "
                f"Original error: {e}"
            )
        except Exception as e:
            return False, f"tradingagents failed to load: {e}"
        return True, ""
    ...
    def _try_cache_hit(self, stock) -> AnalysisResult | None:
        ...
        return AnalysisResult(
            agent_name=self.name,
            title=history.title or f"Deep Analysis · Cached · {stock.name}({stock.symbol})",
