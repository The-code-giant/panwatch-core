...
def backfill_tradingagents_suggestions(days: int = 7) -> dict:
    ...
    try:
        ...
        for r in records:
            ...
            # Infer market (not stored on the analysis row): Canadian Yahoo suffix first,
            # legacy numeric CN/HK codes only matter for old rows, everything else is US.
            symbol = r.stock_symbol
            if symbol.upper().endswith((".TO", ".V", ".NE", ".CN")):
                market = "CA"
