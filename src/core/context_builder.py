...
# Benchmark index per market for relative-strength (code + label).
# US: S&P 500 (.INX → Yahoo ^GSPC); CA: S&P/TSX Composite (GSPTSE → Yahoo ^GSPTSE).
# Retired markets (CN/HK) stay mapped only while enabled, so their branches are gated.
_INDEX_BY_MARKET: dict[str, tuple[str, str]] = {
    "US": (".INX", "S&P 500"),
    "CA": ("GSPTSE", "S&P/TSX Composite"),
}
_LEGACY_INDEX_BY_MARKET: dict[str, tuple[str, str]] = {
    "CN": ("000300", "CSI 300"),
    "HK": ("HSI", "Hang Seng"),
}
for _m, _idx in _LEGACY_INDEX_BY_MARKET.items():
    if is_enabled(_m):
        _INDEX_BY_MARKET[_m] = _idx
# Fallback index for the legacy CN market when 000300 is unavailable (Shanghai Composite).
_CN_INDEX_FALLBACK: tuple[str, str] = ("000001", "Shanghai Composite")
...
class ContextBuilder:
    ...
    def _index_for_market(market) -> tuple[str, str]:
        """market -> (index code, label). Unknown markets fall back to the default market's index."""
        mkt = market.value if isinstance(market, MarketCode) else str(market or "")
        return _INDEX_BY_MARKET.get(mkt) or _INDEX_BY_MARKET.get(default_market()) or ("", "")
    ...
    def _get_index_context(self, market) -> dict | None:
        ...
        # Legacy CN only: fall back to the Shanghai Composite when 000300 is unavailable.
        if (not ctx or not ctx.get("available")) and mkt == "CN" and is_enabled("CN"):
    ...
    async def build_symbol_contexts(
        ...
        *,
        ...
        for stock in context.watchlist:
            ...
            # ① Corporate events (earnings / ex-dividend / splits) as-is; no full-text enrichment
            events_for_payload = [
