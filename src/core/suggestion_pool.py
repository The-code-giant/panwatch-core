...
def get_suggestions_for_stock(
    ...
    try:
        ...
        if stock_market:
            query = query.filter(
                StockSuggestion.stock_market == (stock_market or default_market()).strip().upper()
            )
...
def get_latest_suggestions(
    ...
    try:
        ...
        for s in suggestions:
            key = f"{(s.stock_market or default_market()).upper()}:{s.stock_symbol}"
...
def _to_dict(suggestion: StockSuggestion, now: Optional[datetime] = None) -> dict:
    ...
    return {
        ...
        "stock_market": suggestion.stock_market or default_market(),
