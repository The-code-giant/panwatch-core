"""Strategy layer: signal generation, outcome evaluation, weight tuning, and statistics."""
...
from src.models.market import ENABLED_MARKETS, EQUITY_MARKETS, default_market, is_enabled

logger = logging.getLogger(__name__)


SOURCE_POOL_LABELS = {
    "watchlist": "Watchlist Pool",
    "market_scan": "Market Pool",
    "mixed": "Market + Watchlist",
}


def _resolve_market_scope(market: str) -> tuple[str, list[str] | None]:
    """Normalize a requested market and resolve the eligible-market scope.

    Returns ``(normalized_market, eligible_scope)``. ``eligible_scope`` is the
    list of ``stock_market`` values a query should be restricted to.
    ``eligible_scope`` is ``None`` when the caller named an explicit market
    that is not currently enabled (disabled/unknown/legacy) -- callers must
    treat this as an ineligible request and return the empty result shape
    rather than silently falling back to scanning every market.
    """
    mkt = (market or "").strip().upper()
    if mkt:
        if mkt not in ENABLED_MARKETS:
            return mkt, None
        return mkt, [mkt]
    return "", list(ENABLED_MARKETS)
    ...
    *,
    ...
    if is_holding:
        ...
        if act == "add":
            ...
            act = "watch"
            label = "Watch"
        if label in ("Hold", "Continue holding", "持有", "继续持有"):
            act = "watch"
            label = "Watch"
...
# Per-market caps; markets not listed fall back to the US values via .get(..., default).
...
def _to_market(value: str | None) -> MarketCode:
    try:
        return MarketCode((value or default_market()).strip().upper())
    except Exception:
        return MarketCode(default_market())
...
def _build_market_regime_rows(
    *,
    ...
    for row in candidates:
        mkt = (row.stock_market or default_market()).strip().upper() or default_market()
...
def _build_cross_section_features(candidates: list[EntryCandidate]) -> dict[int, dict]:
    by_market: dict[str, list[EntryCandidate]] = {}
    for c in candidates:
        market = (c.stock_market or default_market()).strip().upper() or default_market()
...
def _apply_portfolio_constraints(*, rows: list[StrategySignalRun]) -> dict:
    ...
    for r in rows:
        m = (r.stock_market or default_market()).strip().upper() or default_market()
    ...
    for market, market_rows in by_market.items():
        ...
        for idx, row in enumerate(active_unheld):
            if idx < max_unheld:
                continue
            _demote_signal(row, reason=f"Portfolio constraint: {market} unheld-opportunity cap exceeded ({max_unheld})")
            demoted += 1
        ...
        for idx, row in enumerate(high_rows):
            if idx < allow_high:
                continue
            _demote_signal(row, reason=f"Portfolio constraint: {market} high-risk share exceeded ({int(max_ratio*100)}%)")
            demoted += 1
        ...
        for code, srows in by_strategy.items():
            ...
            for idx, row in enumerate(srows):
                if idx < cap_per_strategy:
                    continue
                _demote_signal(row, reason=f"Portfolio constraint: {market} strategy {code} concentration too high")
                demoted += 1
...
def _compute_factor_breakdown(
    *,
    ...
    if ("突破" in signal_text) or ("breakout" in signal_text):
        catalyst_score += 2.5
    if ("回踩" in signal_text) or ("pullback" in signal_text):
        catalyst_score += 1.5
    if ("超跌" in signal_text) or ("oversold" in signal_text):
        catalyst_score += 1.0
...
def _sync_factor_and_risk_snapshots(
    *,
    ...
    by_market: dict[str, list[StrategySignalRun]] = {}
    for s in signals:
        market = (s.stock_market or default_market()).strip().upper() or default_market()
...
def _format_signal(
    ...
    *,
    ...
    return {
        ...
        "stock_name": _english_name(row.stock_symbol, row.stock_name or row.stock_symbol),
    ...
    *,
    ...
    try:
            ...
            for code in codes:
                ...
                if not row:
                    row = StrategySignalRun(
                        ...
                        stock_market=market,
                        stock_name=_english_name(c.stock_symbol, c.stock_name or c.stock_symbol),
                        strategy_code=code,
        ...
        if constraint_stats.get("demoted", 0) > 0:
            logger.info(
                "[Strategy layer] Portfolio constraints applied: snapshot=%s demoted=%s details=%s",
                snapshot,
...
def list_strategy_signals(
    *,
    ...
    try:
        snapshot = (snapshot_date or "").strip()
        mkt, scope = _resolve_market_scope(market)
        if scope is None:
            # Explicit but disabled/retired market: never surface legacy rows.
            return {"snapshot_date": snapshot, "count": 0, "items": []}
        if not snapshot:
            latest = (
                db.query(StrategySignalRun.snapshot_date)
                .filter(StrategySignalRun.stock_market.in_(scope))
        ...
        q = db.query(StrategySignalRun).filter(
            ...
            StrategySignalRun.stock_market.in_(scope),
        )
    ...
    try:
        ...
        for s in signals:
            ...
            key = (
                ...
                (s.stock_market or default_market()).strip().upper(),
            )
...
def rebalance_strategy_weights(
    *,
    ...
    try:
        ...
        db.rollback()
        logger.warning(f"Strategy weight tuning failed: {e}")
        return {
            "window_days": window_days,
            "min_samples": min_samples,
            "alpha": alpha,
...
def get_strategy_stats(*, days: int = 45) -> dict:
    ...
    try:
        latest_snapshot_row = (
            db.query(StrategySignalRun.snapshot_date)
            .filter(StrategySignalRun.stock_market.in_(ENABLED_MARKETS))
            .order_by(StrategySignalRun.snapshot_date.desc())
            .first()
        )
        ...
        if snapshot:
            total_signals = (
                ...
                .filter(
                    StrategySignalRun.snapshot_date == snapshot,
                    StrategySignalRun.stock_market.in_(ENABLED_MARKETS),
                )
                .scalar()
            ) or 0
            active_signals = (
            ...
            excluded_signals = (
                ...
                .filter(
                    StrategySignalRun.snapshot_date == snapshot,
                    or_(
                        StrategySignalRun.stock_market.is_(None),
                        StrategySignalRun.stock_market.notin_(ENABLED_MARKETS),
                    ),
                )
                .scalar()
            ) or 0
        ...
        outcome_rows = (
            ...
            .filter(
                StrategyOutcome.created_at >= since,
                StrategyOutcome.outcome_status.in_(("evaluated", "hit_target", "hit_stop")),
                StrategyOutcome.stock_market.in_(ENABLED_MARKETS),
            )
            .group_by(
                StrategyOutcome.strategy_code,
                StrategyOutcome.stock_market,
        ...
        excluded_outcomes = (
            db.query(func.count(StrategyOutcome.id))
            .filter(
                StrategyOutcome.created_at >= since,
                StrategyOutcome.outcome_status.in_(("evaluated", "hit_target", "hit_stop")),
                or_(
                    StrategyOutcome.stock_market.is_(None),
                    StrategyOutcome.stock_market.notin_(ENABLED_MARKETS),
                ),
            )
            .scalar()
        ) or 0
        ...
        if snapshot:
            ...
            # StrategyFactorSnapshot carries its own stock_market column, so we
            # scope it directly rather than joining through signal_run_id.
            factor_rows = (
                db.query(
                ...
                .filter(
                    StrategyFactorSnapshot.snapshot_date == snapshot,
                    StrategyFactorSnapshot.stock_market.in_(ENABLED_MARKETS),
                )
                .first()
            )
            if factor_rows:
            ...
            regimes = (
                ...
                .filter(
                    MarketRegimeSnapshot.snapshot_date == snapshot,
                    MarketRegimeSnapshot.market.in_(ENABLED_MARKETS),
                )
            ...
            risks = (
                ...
                .filter(
                    PortfolioRiskSnapshot.snapshot_date == snapshot,
                    PortfolioRiskSnapshot.market.in_(ENABLED_MARKETS),
                )
        ...
        return {
            ...
            "excluded_by_scope": {
                "signals": int(excluded_signals),
                "outcomes": int(excluded_outcomes),
            },
            "coverage": coverage,
            "constraints": {
...
def list_market_regime_snapshots(
    *,
    ...
    try:
        ...
        scope_norm = (scope or "active").strip().lower()
        if scope_norm != "all":
            scope_norm = "active"
            q = q.filter(MarketRegimeSnapshot.market.in_(ENABLED_MARKETS))
        rows = (
            q.order_by(
...
def list_portfolio_risk_snapshots(
    *,
    ...
    try:
        ...
        if scope_norm != "all":
            scope_norm = "active"
            q = q.filter(PortfolioRiskSnapshot.market.in_(ENABLED_MARKETS))
        rows = (
            q.order_by(
