...
from src.models.market import EQUITY_MARKETS, default_market

logger = logging.getLogger(__name__)

# 模拟盘交易成本(Phase 1)。与回测共用同一成本模型。
COST_MODEL = CostModel()

# 建仓股数下限(一手 100 股;US/CA 可零股,但保持 100 作为最小仓位单位)
FIXED_QUANTITY = 100
...
# 模拟盘可投入的市场 = 已启用的股票市场(单一真相源:src/models/market.EQUITY_MARKETS)。
ALL_MARKETS: tuple[str, ...] = tuple(EQUITY_MARKETS)

# 各市场的默认权重(只对 ALL_MARKETS 中的市场生效,归一化到合计 1.0;默认 US 60 / CA 40)。
_BASE_DEFAULT_WEIGHTS: dict[str, float] = {"US": 0.6, "CA": 0.4, "CN": 0.5, "HK": 0.3}


def _default_allocations() -> dict[str, float]:
    weights = {m: _BASE_DEFAULT_WEIGHTS.get(m, 0.0) for m in ALL_MARKETS}
    total = sum(weights.values())
    if total <= 0:
        return {m: (1.0 if m == default_market() else 0.0) for m in ALL_MARKETS}
    return {m: round(v / total, 6) for m, v in weights.items()}


DEFAULT_ALLOCATIONS: dict[str, float] = _default_allocations()


def _clamp_ratio(v) -> float:
    try:
        return min(1.0, max(0.0, float(v or 0.0)))
    except Exception:
        return 0.0


def normalize_allocations(raw: dict | None) -> dict[str, float]:
    """补齐已启用市场、clamp 到 [0,1],返回 {market: ratio}(键恒为 ALL_MARKETS)。

    已停用市场(如旧库里的 CN/HK)的比例视为 0:它们承载的权重会按比例
    重新分配给已启用市场,使总投入比例不因市场退役而缩水。仅当输入里确实
    含有停用市场的正权重时才重新分配;用户只配置了启用市场时原样保留。
    """
    raw = raw or {}
    out: dict[str, float] = {m: _clamp_ratio(raw.get(m, 0.0)) for m in ALL_MARKETS}
    disabled_weight = sum(
        _clamp_ratio(v) for k, v in raw.items() if str(k).upper() not in ALL_MARKETS
    )
    enabled_weight = sum(out.values())
    if disabled_weight > 0 and enabled_weight > 0:
        target_total = min(1.0, enabled_weight + disabled_weight)
        scale = target_total / enabled_weight
        out = {m: round(min(1.0, v * scale), 6) for m, v in out.items()}
    return out
