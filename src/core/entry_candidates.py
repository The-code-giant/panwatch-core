...
def _score_suggestion(
    *,
    ...
    if suggestion.signal:
        evidence.append(f"Recommendation signal: {suggestion.signal}")
    if suggestion.reason:
        evidence.append(f"Recommendation basis: {suggestion.reason}")
    ...
    if quote:
        ...
        if pct is not None:
            if 0.5 <= abs(pct) <= 6:
                score += 2
                evidence.append(f"Moderate intraday move ({pct:+.2f}%)")
            elif abs(pct) >= 10:
                score -= 3
                evidence.append(f"Excessive intraday move ({pct:+.2f}%)")
    ...
    if kline:
        ...
        if vol_ratio is not None:
            if vol_ratio >= 2:
                score += 4
                evidence.append(f"Volume ratio surging ({vol_ratio:.1f}x)")
            elif vol_ratio >= 1.3:
                score += 2
                evidence.append(f"Volume ratio moderately up ({vol_ratio:.1f}x)")
    ...
    if quality is not None:
        ...
        score += quality_bonus
        evidence.append(f"Context quality score {quality:.0f}")
    ...
    if created_at:
        try:
            ...
            if hours <= 6:
                score += 3
                evidence.append("Recent recommendation (within 6h)")
            elif hours >= 48:
                score -= 3
                evidence.append("Recommendation is stale (48h+)")
        except Exception:
            pass
    ...
    *,
    ...
    if price is not None:
        ...
        if action in ("buy", "add"):
            ...
            invalidation = f"Invalidated if it breaks below {stop_loss:.2f}"
            ...
            invalidation = f"Shift to defensive if it breaks below {stop_loss:.2f}"
        else:
            invalidation = "Currently risk-control focused; opening a new position is not recommended"
...
def _load_holding_keys() -> set[str]:
    ...
    try:
        ...
        return {
            f"{(m or default_market()).strip().upper()}:{(s or '').strip()}"
            for m, s in rows
            if s
        }
    finally:
...
def _derive_market_scan_decision(quote: dict | None, kline: dict | None) -> dict:
    ...
    signal = ", ".join(_strategy_labels(tags[:3])) if tags else "No clear signal yet"
    reason = ", ".join(reasons[:3]) if reasons else "Waiting for a higher-confidence signal before entry"
    return {
        "action": action,
...
def _score_market_scan_candidate(
    *, action: str, quote: dict | None, kline: dict | None, strategy_tags: list[str] | None
    ...
    if tags:
        ...
        evidence.append("Strategy signal: " + " / ".join(_strategy_labels(tags[:3])))
    ...
    if pct is not None:
        if 1 <= pct <= 7:
            ...
            score -= 3
            evidence.append(f"Overheated gain ({pct:+.2f}%)")
...
def _load_market_scan_history_inputs(
    *, market: str, limit: int, max_days: int = 7
    ...
    mkt = (market or default_market()).strip().upper()
...
def _persist_market_scan_snapshot(snapshot: str, market_scan_map: dict[str, dict]) -> None:
    ...
    try:
        ...
        for item in rows:
            ...
            market = str(item.get("market") or default_market()).strip().upper() or default_market()
            if not symbol:
                continue
            quote = item.get("quote_seed") if isinstance(item.get("quote_seed"), dict) else {}
            meta = item.get("meta") if isinstance(item.get("meta"), dict) else {}
            db.add(
                MarketScanSnapshot(
                    ...
                    stock_symbol=symbol,
                    stock_market=market,
                    stock_name=_english_name(symbol, str(item.get("stock_name") or symbol).strip()),
    ...
    try:
        ...
        for key, inp in input_map.items():
            ...
            row = EntryCandidate(
                stock_symbol=symbol,
                stock_market=market,
                stock_name=_english_name(symbol, (inp.get("stock_name") or symbol).strip()),
                snapshot_date=snapshot,
                status=status,
                score=score,
        ...
        try:
            ...
            if not snapshot:
                latest = (
                    ...
                    .filter(EntryCandidate.stock_market.in_(scope))
            ...
            q = db.query(EntryCandidate).filter(
                EntryCandidate.snapshot_date == snapshot,
                EntryCandidate.stock_market.in_(scope),
            )
...
def save_entry_candidate_feedback(
    *,
    ...
    market = (stock_market or default_market()).strip().upper() or default_market()
    if not symbol:
        return False
    ...
    *,
    ...
    try:
        ...
        for c in candidates:
            ...
            key = ((c.stock_symbol or "").strip(), (c.stock_market or default_market()).strip().upper())
...
def get_entry_candidate_stats(*, days: int = 30) -> dict:
    ...
    try:
        ...
        latest_snapshot_row = (
            db.query(EntryCandidate.snapshot_date)
            .filter(EntryCandidate.stock_market.in_(ENABLED_MARKETS))
            .order_by(EntryCandidate.snapshot_date.desc())
            .first()
        )
        ...
        if latest_snapshot:
            ...
            total_active = (
                ...
                .filter(
                    ...
                    EntryCandidate.stock_market.in_(ENABLED_MARKETS),
                    EntryCandidate.status == "active",
                )
                .scalar()
            ) or 0
        ...
        for m, cnt, u in by_market_rows:
            cnt = int(cnt or 0)
            u = int(u or 0)
            m_norm = (m or default_market()).strip().upper()
            by_market.append(
                {
                    "market": m_norm,
                    "supported": is_enabled(m_norm),
                    "total": cnt,
                    "useful": u,
                    "useful_rate": round((u / cnt * 100.0), 2) if cnt > 0 else 0.0,
                }
