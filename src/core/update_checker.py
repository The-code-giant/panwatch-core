...
def _human_error(err: str | None) -> str | None:
    ...
    mapping = {
        ...
        "not_configured": "Update checking is disabled (set UPDATE_CHECK_DOCKER_REPO to enable)",
        "invalid_repo": "Invalid update-check configuration",
        "no_semver_tag": "No usable version tag found",
        "hub_timeout": "Timed out connecting to Docker Hub",
        "hub_unreachable": "Network unreachable — cannot connect to Docker Hub",
        "hub_request_failed": "Docker Hub request failed",
        "registry_timeout": "Timed out connecting to Docker Registry",
        "registry_unreachable": "Network unreachable — cannot connect to Docker Registry",
        "registry_request_failed": "Docker Registry request failed",
        "registry_auth_no_token": "Docker Registry authentication failed (no token)",
    ...
    if code.startswith("registry_auth_http_"):
        return f"Docker Registry authentication error (HTTP {code.replace('registry_auth_http_', '')})"
...
def check_update(current_version: str, proxy: str | None = None) -> dict[str, object]:
    ...
    if force_disable or not repo:
        return {
            ...
            "release_url": f"https://hub.docker.com/r/{repo}/tags" if repo else None,
