...
class ContextMaintenanceScheduler:
    ...
    def start(self):
        ...
        # 机会自动刷新 —— 06:15 盘前 / 13:30 收盘后 / 19:00 晚间。
        # 时间点按调度器时区(app_timezone,默认 America/Vancouver,美股 06:30-13:00)解释,与 Agent cron 语义一致。
        for job_hour, job_minute in ((6, 15), (13, 30), (19, 0)):
            self.scheduler.add_job(
