...
def _stock_to_response(stock: Stock) -> dict:
    return {
        ...
        "supported": is_enabled(stock.market),
        "agents": [
            {
...
def get_market_status():
    ...
    for market_code, market_def in MARKETS.items():
        if not is_enabled(market_code):
            continue  # retired markets (CN/HK) are not shown
        try:
            ...
            if is_trading:
                ...
                # Determine pre-market vs after-hours
...
def refresh_list():
    """Force a refresh of the tradable universe (US + CA directories)."""
    stats = refresh_stock_list()
    refreshed = stats.get("refreshed") or {}
    refreshed_at = max((float(v) for v in refreshed.values() if v), default=None)
    return {
        "count": int(stats.get("count") or 0),
        "by_market": dict(stats.get("by_market") or {}),
...
def delete_stock(stock_id: int, db: Session = Depends(get_db)):
    ...
    if has_position:
        raise HTTPException(400, "This stock has open positions. Delete the positions before deleting the stock.")
...
def update_stock_agents(stock_id: int, body: StockAgentUpdate, db: Session = Depends(get_db)):
    ...
    for item in body.agents:
        ...
        if agent_kind != AGENT_KIND_WORKFLOW:
            raise HTTPException(400, f"Agent {item.agent_name} is an internal capability and cannot be bound to a stock")
...
async def trigger_stock_agent(
    ...
    if stock_id > 0:
        ...
        if not sa and not allow_unbound:
            raise HTTPException(400, f"Stock is not linked to agent {agent_name}")
        ...
        market = (market or default_market()).strip().upper() or default_market()
    ...
    if agent_name == "tradingagents" and not force_refresh:
        ...
        if existing_trace:
            ...
            return {
                ...
                "message": "A deep analysis is already running; returning existing task progress",
                "deduplicated": True,
            }
    ...
    if not wait:
        ...
        t.start()
        return {"queued": True, "trace_id": trace_id, "message": "Submitted for background execution"}
