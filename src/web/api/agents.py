...
async def trigger_agent_endpoint(
    ...
    try:
        ...
        if (
            ...
            return {"ok": True, "queued": True, "message": "Submitted for background execution"}
...
def get_tradingagents_history_comparison(
    stock_symbol: str = Query(..., description="Stock symbol, e.g. NVDA or SHOP.TO"),
    market: str = Query("", description="Market: US/CA"),
...
def get_tradingagents_budget(db: Session = Depends(get_db)):
    ...
    if not agent:
        raise HTTPException(404, "tradingagents agent is not registered")
...
async def scan_intraday(analyze: bool = False, db: Session = Depends(get_db)):
    ...
    if not watchlist:
        return {
            "stocks": [],
            "message": "Please enable the \"Intraday Monitor\" agent for this stock first",
            "scanned_count": 0,
            "has_watchlist": False,
        }
    ...
    async def _build_result_item(quote):
        ...
        market = stock_market_map.get(quote.symbol, MarketCode.UNKNOWN)

        # 获取持仓信息
        positions = portfolio.get_positions_for_stock(quote.symbol, market)
                ...
                try:
                    async with ai_sem:
                        ...
                        save_suggestion(
                            ...
                            stock_market=item.get("market") or default_market(),
                            meta={
