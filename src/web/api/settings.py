...
from src.core.stock_link import DEFAULT_PLATFORM
...
# Setting descriptions (shown as labels in Settings -> System)
SETTING_DESCRIPTIONS = {
    "http_proxy": "HTTP proxy address (once set, all outbound requests — quotes/news/AI/notifications — route through it)",
    "notify_quiet_hours": "Notification quiet hours (HH:MM-HH:MM, blank to disable)",
    "notify_retry_attempts": "Notification retry attempts (not counting the first try)",
...
def list_settings(db: Session = Depends(get_db)):
    ...
    for key in SETTING_KEYS:
        ...
        if key not in existing_map:
            ...
            s = existing_map[key]
            # description is app-defined copy, not user data — always sync from the current catalog
...
def set_avatar(update: SettingUpdate, db: Session = Depends(get_db)):
    ...
    if not (value.startswith("data:") and "," in value):
        raise HTTPException(400, "Avatar must be a data URL")
    ...
    with open(os.path.join(_avatar_dir(), fname), "wb") as f:
        f.write(raw)
    if old and old != fname:  # extension changed -> clean up the old file
        try:
            os.remove(os.path.join(_avatar_dir(), old))
        except OSError:
            pass
    if not row:
        row = AppSettings(key=AVATAR_KEY, value=fname, description="User avatar filename")
        db.add(row)
    else:
        row.value = fname
    db.commit()
