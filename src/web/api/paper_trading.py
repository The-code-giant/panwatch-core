...
class UpdateSettingsBody(BaseModel):
    ...
    market_allocations: dict[str, float] | None = None  # {"US":0.6,"CA":0.4}，合计 ≤ 1
...
async def close_position(position_id: int):
    ...
    if not result.get("ok"):
        raise HTTPException(400, result.get("error", "Failed to close position"))
    return {"ok": True}
...
def update_settings(body: UpdateSettingsBody, db: Session = Depends(get_db)):
    ...
    if body.market_allocations is not None:
        ...
        if total > 1.0 + 1e-9:
            raise HTTPException(400, f"Allocation total cannot exceed 100% (currently {round(total * 100)}%)")
