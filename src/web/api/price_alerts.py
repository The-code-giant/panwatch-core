...
def _validate_condition_group(group: AlertConditionGroup):
    if group.op not in ("and", "or"):
        raise HTTPException(400, "condition_group.op only supports and/or")
    if not group.items:
        raise HTTPException(400, "condition_group.items cannot be empty")
    allowed_types = {"price", "change_pct", "turnover", "volume", "volume_ratio"}
    allowed_ops = {">=", "<=", ">", "<", "==", "=", "!=", "<>", "between", "in"}
    for it in group.items:
        if it.type not in allowed_types:
            raise HTTPException(400, f"Unsupported condition type: {it.type}")
        if it.op not in allowed_ops:
            raise HTTPException(400, f"Unsupported operator: {it.op}")
        if it.op in ("between", "in"):
            if not isinstance(it.value, list) or len(it.value) != 2:
                raise HTTPException(400, f"{it.type} with operator {it.op} requires two values")
...
def create_alert_rule(body: PriceAlertCreate, db: Session = Depends(get_db)):
    ...
    if body.expire_at:
        try:
            ...
            raise HTTPException(400, "Invalid expire_at format")
