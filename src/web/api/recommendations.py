...
def get_strategy_regimes(
    ...
    scope: str = Query(
        "active",
        description="Result scope: 'active' restricts to currently enabled markets, 'all' returns full history including legacy markets.",
    ),
):
...
def refresh_strategy_signal_list(
    ...
    return {
        ...
        "message": "Submitted for background execution" if started else "A refresh task is already running",
