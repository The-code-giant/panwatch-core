...
from src.models.market import default_market as _default_market

# Column default for market fields: the deployment's default equity market (US).
...
class Stock(Base):
    ...
    name = Column(String, nullable=False)
    market = Column(String, nullable=False)  # US / CA / CRYPTO / GOLD
...
class DataSource(Base):
    ...
    )  # "news" / "chart" / "quote" / "kline" / "events" / "flash_news" / "fundamentals" / "dividend" / "holders" / "filings"
    provider = Column(String, nullable=False)  # "yfinance" / "google_news" / "sec_edgar" / "tencent" ...
...
class NewsCache(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    source = Column(String, nullable=False)  # vendor key, e.g. "cnbc" / "yfinance"
...
class StockContextSnapshot(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    symbol = Column(String, nullable=False)
    market = Column(String, nullable=False)  # US/CA
...
class AgentPredictionOutcome(Base):
    ...
    agent_name = Column(String, nullable=False)
    stock_symbol = Column(String, nullable=False)
    stock_market = Column(String, nullable=False, default=DEFAULT_MARKET)
...
class StockSuggestion(Base):
    ...
    stock_market = Column(String, nullable=False, default=DEFAULT_MARKET, index=True)
...
class StrategyCatalog(Base):
    ...
    enabled = Column(Boolean, default=True)
    market_scope = Column(String, default="ALL")  # ALL/US/CA
...
class FactorWeight(Base):
    ...
    market = Column(String, nullable=False, default=DEFAULT_MARKET)  # US/CA
    weight = Column(Float, nullable=False, default=1.0)
...
class FactorWeightHistory(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    factor_code = Column(String, nullable=False)
    market = Column(String, nullable=False, default=DEFAULT_MARKET)
    old_weight = Column(Float, nullable=False, default=1.0)
    new_weight = Column(Float, nullable=False, default=1.0)
...
class PaperTradingAccount(Base):
    ...
    enabled = Column(Boolean, default=True)
    excluded_markets = Column(JSON, default=[])  # 排除的市场，如 ["CA"]（兼容旧字段，由 market_allocations 派生）
    # 各市场投资比例 {"US":0.6,"CA":0.4}，比例 0~1、合计 ≤ 1；比例 0 表示不投入该市场
